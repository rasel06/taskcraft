...
export function ProjectMembersManager({
  ...
  async function setRole(userId: string, role: string) {
    try {
      await updateProjectMemberRole(projectId, userId, role as "ADMIN" | "MEMBER");
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm text-foreground">
                {m.user.name}
                {m.userId === leadId && <span className="ml-1.5 text-xs text-faint-foreground">(Lead)</span>}
              </div>
              <div className="truncate text-xs text-muted-foreground">{m.user.email}</div>
            </div>
            <Select value={m.role} onValueChange={(v) => setRole(m.userId, v)} disabled={!canManage}>
