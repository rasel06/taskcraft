...
### Issue statuses (preset library + per-project workflows)
- `IssueStatusPreset` is a workspace library of issue statuses (name, color, category, order, "preselected for new projects"). Seeded with Backlog, Todo, In Progress, In Review, Testing, Blocked, Done, Cancelled; the five classic ones are preselected.
- Each project's workflow is `IssueStatus` rows linked to presets (`presetId`), with the project's own column order and default status, so the number of statuses varies per project. `Issue.status` stores the status name.
- Everything is admin-only (`manage_issue_statuses`), managed at Settings → Projects → Statuses: **Project workflows** (pick a project; add a preset, drag to reorder, ★ default, remove with issues moved to another status), **Status presets** (create, rename/recolor — carried to every project using it and to their issues —, reorder, toggle "New projects", delete when unused) and **Project lifecycle** (project statuses). A project's own settings page has the same workflow editor, and admins can reorder/remove/add columns from the project board (column "…" menu and "Add status" from presets). `/settings/issues/statuses` redirects to the new page.
- Creating a project includes an "Issue workflow" picker (preselected presets ticked, ★ default); `createProject` creates the workflow rows with the project.
- Boards spanning projects (My Issues, team, cycle, views) show the merged status names of the projects involved and are read-only for workflow changes. Cycle/report progress uses each project's completed/canceled categories.
- Code: presets and "add to project" in `src/actions/issue-status-presets.ts`; reorder/default/remove within a project in `src/actions/statuses.ts`; UI in `src/components/settings/issue-workflow.tsx`; client access via `IssueStatusesProvider` / `ProjectIssueStatusesProvider`.
...
- Issue cards have a Timeline button (calendar-clock icon, next to the discussion count) that opens a dialog with a summary (current status/priority/assignees, age, time in current status, status-change and comment counts, contributors), a status-journey bar showing time spent in each status, filter tabs (All / Status / Comments / Assignment / Edits) and the full history, oldest first and grouped by day with before→after chips, assignee/label/attachment diffs, quoted comments and relative times: creation, every field change (status, priority, assignees, labels, milestone, cycle, title, description, attachments) and comment events. It loads on demand via `getIssueTimeline` in `src/actions/issues.ts` (checks project access, resolves user/milestone/cycle names). The attachment-count button was removed from cards.
